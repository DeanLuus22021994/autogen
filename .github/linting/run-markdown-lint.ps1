...
function Invoke-MarkdownLinting {
    ...
    # Don't use the config file which has issues with rule paths
    # Instead pass the rules directly as parameters
    #$command += " --config `"$configPath`""
    $command += " --config '{\"default\": true, \"MD013\": { \"line_length\": 120 }, \"MD033\": false, \"MD041\": false}'"
...
try {
    # Ensure rules directory is set up
    $rulesDir = Join-Path -Path $scriptPath -ChildPath "rules"
    Ensure-RulesExist -RulesDirectory $rulesDir

    # Run markdown linting
    if ($Fix) {
        Write-Host "Running markdown linting with auto-fix..." -ForegroundColor Cyan
        $success = Run-MarkdownLint -Fix -Path $Path
    }
    else {
        Write-Host "Running markdown linting..." -ForegroundColor Cyan
        $success = Run-MarkdownLint -Path $Path
    }
